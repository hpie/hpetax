...
//    'host' => 'localhost'
//);
$where =" tax_dealer_code='' OR tax_dealer_password='' "; 
if(!empty($_REQUEST['search']['value'])){
    $value=$_REQUEST['search']['value'];
    $where.=" AND (tax_dealer_name LIKE '%$value%' OR tax_dealer_code LIKE '%$value%' OR tax_dealer_tin LIKE '%$value%' OR tax_dealer_tin_expiry LIKE '%$value%' OR tax_dealer_mobile LIKE '%$value%') ";
}
//echo $where;die;
...
echo json_encode(
    SSP::pendingDealerList($_REQUEST, $sql_details, $table, $primaryKey, $columns,$where)
);
